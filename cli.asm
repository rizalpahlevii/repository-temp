...
get_cmd:				; Main processing loop
	...
	; My Custom Command
	mov di, string_name ;'NAMA' entered ? 
	call os_string_compare
	jc print_name

	mov di, string_nim ;'NIM' entered ? 
	call os_string_compare
	jc print_nim

	mov di, string_address ;'ALAMAT' entered ? 
	call os_string_compare
	jc print_address
	
	mov di, string_birth_date ;'TANGGALLAHIR' entered ? 
	call os_string_compare
	jc print_birth_date
	
	mov di, string_age ;'UMUR' entered ? 
	call os_string_compare
	jc print_age
	
	mov di, string_hobby ;'HOBI' entered ? 
	call os_string_compare
	jc print_hobby

	mov di, string_phone ;'HP' entered ? 
	call os_string_compare
	jc print_phone
	
	mov di, string_ambition ;'CITACITA' entered ? 
	call os_string_compare
	jc print_ambition
	
	mov di, string_university ;'UNIVERSITAS' entered ? 
	call os_string_compare
	jc print_university
	
	mov di, string_major ;'JURUSAN' entered ? 
	call os_string_compare
	jc print_major
...
; =====================================================================
	...
	help_text		db 'Commands: DIR, LS, COPY, REN, DEL, CAT, SIZE, CLS, HELP, TIME, DATE, VER, EXIT, NAMA, NIM, ALAMAT, UNIVERSITAS, UMUR, JURUSAN, HOBI, CITACITA, TANGGALLAHIR, HP', 13, 10, 0
	...
	; My custom commands
	string_name 	db 'NAMA', 0
	string_nim 	db 'NIM', 0
	string_address 	db 'ALAMAT', 0
	string_birth_date 	db 'TANGGALLAHIR', 0
	string_phone 	db 'HP', 0
	string_ambition 	db 'CITACITA', 0
	string_age 	db 'UMUR', 0
	string_hobby 	db 'HOBI', 0
	string_university 	db 'UNIVERSITAS', 0
	string_major 	db 'JURUSAN', 0
	;End My custom commands


	; My custom variables
	v_name 			db 'Muhammad Rizal Pahlevi',13,10,0
	v_nim			db 'A11.2020.12692',13,10,0
	v_address		db 'Pati',13,10,0
	v_birth_date	db '13 Juli 2002',13,10,0
	v_phone			db '082158115949',13,10,0
	v_ambition		db 'App Developer',13,10,0
	v_age			db '19 Tahun',13,10,0
	v_hobby			db 'Bermain Game',13,10,0
	v_university	db 'Universitas Dian Nuswantoro ',13,10,0
	v_major			db 'Teknik Informatika',13,10,0
